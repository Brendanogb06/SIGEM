...
const priorityConfig = {
  Baixa: { color: "#6b7280", soft: "#f1f2f4" },
  Média: { color: "#1671e8", soft: "#eaf3ff" },
  Alta: { color: "#f59e0b", soft: "#fff6df" },
  Urgente: { color: "#ef1b24", soft: "#ffebec" },
};

const cancellableStatuses = ["Novo", "Pendente", "Em análise"];
const statusOptions = ["Todos", ...Object.keys(statusConfig)];
const priorityOptions = ["Todas", ...Object.keys(priorityConfig)];
...
const categoryOptions = ["Todas", ...Object.keys(categoryConfig)];
...
// Dados temporários dos chamados, agora com prioridade definida pelo gestor.
const tickets = [
  {
    id: "05231", title: "Computador não liga", status: "Concluído", priority: "Média",
  ...
  {
    id: "09865", title: "Não consigo utilizar o projetor", status: "Novo", priority: "Baixa",
  ...
  {
    id: "03476", title: "Porta quebrada", status: "Pendente", priority: "Média",
  ...
  {
    id: "06105", title: "Impressora não imprime", status: "Em análise", priority: "Baixa",
  ...
  {
    id: "07318", title: "Ar-condicionado com ruído", status: "Em atendimento", priority: "Média",
  ...
  {
    id: "08742", title: "Bebedouro sem refrigeração", status: "Suspenso", priority: "Baixa",
  ...
  {
    id: "04120", title: "Tela com manchas", status: "Cancelado", priority: "Baixa",
    ...
    category: "Tecnologia",
    cancellationReason: "Solicitação cancelada pelo gestor.",
    history: ["created", "viewed", "canceled"],
  },
  {
    id: "06482", title: "Tomada soltando faíscas", status: "Em atendimento", priority: "Urgente",
  ...
  {
    id: "07594", title: "Corredor precisa de limpeza", status: "Pendente", priority: "Alta",
  ...
  {
    id: "08317", title: "Câmera sem imagem", status: "Em análise", priority: "Alta",
  ...
  {
    id: "09126", title: "Cadeira patrimonial danificada", status: "Novo", priority: "Baixa",
  ...
  {
    id: "04673", title: "Montagem de quadro informativo", status: "Suspenso", priority: "Baixa",
...
const openStatuses = ["Novo", "Pendente", "Em análise", "Em atendimento", "Suspenso"];
    ...
    <div className="ticket-details">
      <div className="details-grid">
        <div className="details-column">
          ...
          <InfoBlock icon={AlertTriangle} title="Prioridade" tone={ticket.priority === "Urgente" ? "red" : "orange"}>
            <span
              className="priority-pill"
              style={{ "--priority": priorityConfig[ticket.priority].color, "--priority-soft": priorityConfig[ticket.priority].soft }}
            >
              <i />{ticket.priority}
            </span>
          </InfoBlock>
...
export default function AcompanharChamadosGestor() {
  ...
  const resolvedTickets = tickets.map((ticket) => (
    canceledIds.includes(ticket.id)
      ? { ...ticket, status: "Cancelado", updated: "há poucos segundos", cancellationReason: "Solicitação cancelada pelo gestor.", history: ["created", "viewed", "canceled"] }
      : ticket
  ));

  // Chamados urgentes ainda em aberto, usados para o destaque de notificação.
  const urgentOpenTickets = resolvedTickets.filter(
    (ticket) => ticket.priority === "Urgente" && openStatuses.includes(ticket.status)
  );

  // Aplica busca, status, categoria, prioridade e período.
  const displayedTickets = resolvedTickets
    ...
    .filter((ticket) => priorityFilter === "Todas" || ticket.priority === priorityFilter)
    .filter((ticket) => {
    ...
    .filter((ticket) => {
      ...
      return `${ticket.id} ${ticket.title} ${ticket.description} ${ticket.status} ${ticket.priority}`
        .toLocaleLowerCase("pt-BR")
        .includes(query);
    })
    .sort((a, b) => {
      // Chamados urgentes em aberto sobem para o topo da lista do gestor.
      const aUrgent = a.priority === "Urgente" && openStatuses.includes(a.status) ? 0 : 1;
      const bUrgent = b.priority === "Urgente" && openStatuses.includes(b.status) ? 0 : 1;
